...
function createLlmModel(config: LlmConfig): BaseChatModel {
  ...
  switch (config.provider) {
    ...
    case "openai-compatible":
      if (!config.baseUrl) {
        throw new ConfigError(
          'baseUrl is required for openai-compatible provider. Example: "http://localhost:8000/v1"'
        );
      }
      return new ChatOpenAI({
...
export async function parsePaymentEmail(
  ...
  // Sanitize the from field — extract bare email address to prevent display-name injection
  const fromMatch = email.from.match(/<([^>]+)>/);
  const sanitizedFrom = (fromMatch?.[1] ?? email.from).trim().slice(0, 200);
